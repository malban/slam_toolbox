...
public:
  ...
  ScanManager(kt_int32u runningBufferMaximumSize, kt_double runningBufferMaximumDistance)
    : m_pLastScan(NULL), m_RunningBufferMaximumSize(runningBufferMaximumSize),
      m_RunningBufferMaximumDistance(runningBufferMaximumDistance), m_NextStateId(0)
  {
  }
...
void MapperSensorManager::RegisterSensor(const Name& rSensorName)
{
  ...
  {
    m_ScanManagers[rSensorName] =
      new ScanManager(m_RunningBufferMaximumSize, m_RunningBufferMaximumDistance);
  }
}
                                 ...
                                 kt_double degeneracyPenalty)
{
  ...
  pScanMatcher->m_degeneracyPenalty = degeneracyPenalty;
 ...
 * @param doRefineMatch whether to do finer-grained matching if coarse match is good (default is
 * true)
                                 ...
                                 kt_bool doDegeneracyCheck)
{
  ...
  // 1. get corrected sensor position
  ...
  Vector2<kt_double> coarseSearchOffset(
  ...
  kt_double bestResponse = CorrelateScan(
    pScan, scanPose, coarseSearchOffset, coarseSearchResolution,
    m_pMapper->m_pCoarseSearchAngleOffset->GetValue(),
    m_pMapper->m_pCoarseAngleResolution->GetValue(), doPenalize, rMean, rCovariance, false);
  ...
  if (m_pMapper->m_pUseResponseExpansion->GetValue() == true)
  {
    if (math::DoubleEqual(bestResponse, 0.0))
    {
      ...
      for (kt_int32u i = 0; i < 3; i++)
      {
        ...
        bestResponse = CorrelateScan(
          pScan, scanPose, coarseSearchOffset, coarseSearchResolution, newSearchAngleOffset,
          m_pMapper->m_pCoarseAngleResolution->GetValue(), doPenalize, rMean, rCovariance, false);
  ...
  if (doRefineMatch)
  {
    ...
    bestResponse = CorrelateScan(pScan, rMean, fineSearchOffset, fineSearchResolution,
                                 ...
                                 m_pMapper->m_pFineSearchAngleOffset->GetValue(), doPenalize, rMean,
                                 rCovariance, true);
  }
  ...
  if (doDegeneracyCheck && m_degeneracyThreshold < 1) {
    ...
    // Check if the scan points corresponding to occupied grid cells represent
    // a degenerate scenario where a single axis dominates the correlation
    //
    // This is a heuristic to increase the covariance along the degenerate
    // axis without needing to use a large correlation window.
    ...
    // Calculate point normals of scan with a 20cm baseline.
    auto point_normals = calculatePointNormals(*pScan, 0.2, true);

    // TODO(malban): Consider caching the point normals of the scan since they
    //               will be fixed.  Though, they would need to be adjusted
    //               if the scan is rotated via a correction.

    // Filter normals to only include valid points that correspond to occupied
    // grid cells.
    ...
    // TODO(malban): covariance seems too large in the minor axis in some cases, could be due to slight rotation if the
    //               angle search is too coarse

    // Calculate the principle components of the valid normal vectors.
    // This will result in two orthogonal vectors with lengths scaled proportionally to their representation in the
    // set of normal vectors.
    auto normal_components = GetPrincipalComponents(valid_normals, false);

    // Calculate the degeneracy of the normal components.
    //
    // degeneracy = 1.0 - |minor component| / |major component|
    //
    double degeneracy = std::max(0.0, 1.0 - std::min(1.0, normal_components[1].norm() / normal_components[0].norm()));

    if (degeneracy > m_degeneracyThreshold)
    {
      // Rotate the normal component vectors into the corrected orientation.
      ...
      normal_components[0] = rotation * normal_components[0];
      normal_components[1] = rotation * normal_components[1];

      // Scale the degenerate axis
      Eigen::Vector2d degenerate_axis_scaled = normal_components[1].normalized() * m_degeneracyPenalty;

      // Create degeneracy covariance matrix in world frame.
      ...
      // Add the degeneracy covariance to the correlation covariance.
      rCovariance += cov_mat;
    }
  }
...
void ScanMatcher::match(const kt_double& y) const
{
       ...
       ++xIter)
  {
    ...
    for (kt_int32u angleIndex = 0; angleIndex < m_nAngles; angleIndex++)
    {
      ...
      if (m_doPenalize && (math::DoubleEqual(response, 0.0) == false))
      {
        ...
        kt_double distancePenalty = 1.0 - (DISTANCE_PENALTY_GAIN * squaredDistance /
        ...
        kt_double anglePenalty         = 1.0 - (ANGLE_PENALTY_GAIN * squaredAngleDistance /
      ...
      m_pPoseResponse[poseResponseCounter] = std::pair<kt_double, Pose2>(
        response, Pose2(newPositionX, newPositionY, math::NormalizeAngle(angle)));
    }
  }
}
...
kt_double ScanMatcher::CorrelateScan(LocalizedRangeScan* pScan, const Pose2& rSearchCenter,
                                     ...
                                     kt_bool doPenalize, Pose2& rMean, Matrix3& rCovariance,
                                     kt_bool doingFineMatch)
{
  assert(searchAngleResolution != 0.0);

  // setup lookup arrays
  m_pGridLookup->ComputeOffsets(pScan, rSearchCenter.GetHeading(), searchAngleOffset,
  ...
  Vector2<kt_int32s> startGridPoint = m_pCorrelationGrid->WorldToGrid(
    Vector2<kt_double>(rSearchCenter.GetX() + startX, rSearchCenter.GetY() + startY));
  ...
  {
    ComputeAngularCovariance(averagePose, bestResponse, rSearchCenter, searchAngleOffset,
                                              ...
                                              kt_double searchAngleResolution, Matrix3& rCovariance)
{
  ...
  for (kt_int32u yIndex = 0; yIndex < nY; yIndex++)
  {
    ...
    for (kt_int32u xIndex = 0; xIndex < nX; xIndex++)
    {
      ...
      Vector2<kt_int32s> gridPoint = m_pSearchSpaceProbs->WorldToGrid(
        Vector2<kt_double>(rSearchCenter.GetX() + x, rSearchCenter.GetY() + y));
 ...
 * @param rCovariance
 */
void ScanMatcher::ComputeAngularCovariance(const Pose2& rBestPose, kt_double bestResponse,
                                           const Pose2& rSearchCenter, kt_double searchAngleOffset,
                                           kt_double searchAngleResolution, Matrix3& rCovariance)
{
  ...
  kt_double bestAngle =
    math::NormalizeAngleDifference(rBestPose.GetHeading(), rSearchCenter.GetHeading());
 ...
 */
void ScanMatcher::AddScan(LocalizedRangeScan* pScan, const Vector2<kt_double>& rViewPoint,
                          kt_bool doSmear)
{
 ...
 */
PointVectorDouble ScanMatcher::FindValidPoints(LocalizedRangeScan* pScan,
...
public:
  ...
  explicit BreadthFirstTraversal(Graph<T>* pGraph) : GraphTraversal<T>(pGraph)
  {
  }
...
MapperGraph::MapperGraph(Mapper* pMapper, kt_double rangeThreshold) : m_pMapper(pMapper)
{
  m_pLoopScanMatcher =
    ScanMatcher::Create(pMapper, m_pMapper->m_pLoopSearchSpaceDimension->GetValue(),
                        ...
                        m_pMapper->getParamMatchDegeneracyThreshold(), m_pMapper->getParamMatchDegeneracyPenalty());
...
kt_bool MapperGraph::TryCloseLoop(LocalizedRangeScan * pScan, const Name& rSensorName)
{
  ...
  while (!candidateChain.empty())
  {
    ...
    // decompose the covariance matrix into it's principal components to find the primaray and secondary axes
    auto components = GetPrincipalComponents(Eigen::Matrix2d{ {covariance(0, 0), covariance(0, 1)}, {covariance(1, 0), covariance(1, 1)} });
    auto primary_axis = components[0];
    auto secondary_axis = components[1];
...
{
  std::map<int, Vertex<LocalizedRangeScan>*>::iterator v1 =
    m_Vertices[pSourceScan->GetSensorName()].find(pSourceScan->GetStateId());
  std::map<int, Vertex<LocalizedRangeScan>*>::iterator v2 =
    m_Vertices[pTargetScan->GetSensorName()].find(pTargetScan->GetStateId());
...
void MapperGraph::LinkNearChains(LocalizedRangeScan* pScan, Pose2Vector& rMeans,
  ...
  const_forEach(LocalizedRangeScanVector, &nearLinkedScans)
  {
         ...
         candidateScanNum--)
    {
      LocalizedRangeScan* pCandidateScan =
        m_pMapper->m_pMapperSensorManager->GetScan(pNearScan->GetSensorName(), candidateScanNum);
      ...
      Pose2 candidatePose =
        pCandidateScan->GetReferencePose(m_pMapper->m_pUseScanBarycenter->GetValue());
      kt_double squaredDistance =
    ...
    kt_int32u end = static_cast<kt_int32u>(
      m_pMapper->m_pMapperSensorManager->GetScans(pNearScan->GetSensorName()).size());
...
std::vector<Vertex<LocalizedRangeScan>*> MapperGraph::FindNearByVertices(Name name,
                                                                         const Pose2 refPose,
                                                                         kt_double maxDistance)
{
  ...
  typedef nanoflann::KDTreeSingleIndexAdaptor<nanoflann::L2_Simple_Adaptor<double, P2KD>, P2KD, dim>
    my_kd_tree_t;
...
void MapperGraph::CorrectPoses()
{
  ...
  if (pSolver != NULL)
  {
    ...
    const_forEach(ScanSolver::IdPoseVector, &pSolver->GetCorrections())
    {
      ...
      totalCorrectionHeading +=
        std::abs(math::NormalizeAngleDifference(iter->second.GetHeading(), old_pose.GetHeading()));
      numberOfScans++;
...
{
  m_pUseScanMatching =
    new Parameter<kt_bool>("UseScanMatching",
  ...
  m_pMinimumTravelDistance =
    new Parameter<kt_double>("MinimumTravelDistance",
  ...
  m_pLoopMatchMinimumChainSize =
    new Parameter<kt_int32u>("LoopMatchMinimumChainSize",
  ...
  m_pMatchDegeneracyThreshold = new Parameter<kt_double>(
    "MatchDegenercyThreshold",
  ...
  m_pMatchDegeneracyPenalty = new Parameter<kt_double>(
    "MatchDegenercyPenalty",
  ...
  m_pCorrelationSearchSpaceResolution = new Parameter<kt_double>(
    ...
    "The resolution (size of a grid cell) of the correlation grid.", 0.01, GetParameterManager());

  m_pCorrelationSearchSpaceSmearDeviation =
    new Parameter<kt_double>("CorrelationSearchSpaceSmearDeviation",
  ...
  m_pLoopSearchSpaceDimension = new Parameter<kt_double>(
    "LoopSearchSpaceDimension", "The size of the search grid used by the matcher.", 8.0,
    GetParameterManager());

  m_pLoopSearchSpaceResolution = new Parameter<kt_double>(
    "LoopSearchSpaceResolution", "The resolution (size of a grid cell) of the correlation grid.",
    0.05, GetParameterManager());

  m_pLoopSearchSpaceSmearDeviation =
    new Parameter<kt_double>("LoopSearchSpaceSmearDeviation",
                             "The point readings are smeared by this value in X and Y to create a "
                             "smoother response.",
                             0.03, GetParameterManager());
  ...
  m_pDistanceVariancePenalty =
    new Parameter<kt_double>("DistanceVariancePenalty",
  ...
  m_pAngleVariancePenalty =
    new Parameter<kt_double>("AngleVariancePenalty", "See DistanceVariancePenalty.",
                             math::Square(math::DegreesToRadians(20)), GetParameterManager());

  m_pFineSearchAngleOffset = new Parameter<kt_double>(
    "FineSearchAngleOffset", "The range of angles to search during a fine search.",
    math::DegreesToRadians(0.2), GetParameterManager());

  m_pCoarseSearchAngleOffset = new Parameter<kt_double>(
    "CoarseSearchAngleOffset", "The range of angles to search during a coarse search.",
    math::DegreesToRadians(20), GetParameterManager());

  m_pCoarseAngleResolution = new Parameter<kt_double>(
    "CoarseAngleResolution", "Resolution of angles to search during a coarse search.",
...
double Mapper::getParamMatchDegeneracyThreshold()
{
  return static_cast<double>(m_pMatchDegeneracyThreshold->GetValue());
}

double Mapper::getParamMatchDegeneracyPenalty()
{
  return static_cast<double>(m_pMatchDegeneracyPenalty->GetValue());
}
...
void Mapper::setParamMatchDegeneracyThreshold(double d)
{
  m_pMatchDegeneracyThreshold->SetValue((kt_double)d);
}

void Mapper::setParamMatchDegeneracyPenalty(double d)
{
  m_pMatchDegeneracyPenalty->SetValue((kt_double)d);
}
...
void Mapper::Initialize(kt_double rangeThreshold)
{
  ...
  m_pSequentialScanMatcher =
    ScanMatcher::Create(this, m_pCorrelationSearchSpaceDimension->GetValue(),
                        ...
                        getParamMatchDegeneracyThreshold(), getParamMatchDegeneracyPenalty());
  assert(m_pSequentialScanMatcher);

  if (m_Deserialized)
  {
    m_pMapperSensorManager->SetRunningScanBufferSize(m_pScanBufferSize->GetValue());
    m_pMapperSensorManager->SetRunningScanBufferMaximumDistance(
...
kt_bool Mapper::Process(LocalizedRangeScan* pScan, Matrix3* covariance, bool force_match_only)
{
  if (pScan != NULL)
  {
    ...
    if (m_pUseScanMatching->GetValue() && pLastScan != NULL)
    {
      ...
      m_pSequentialScanMatcher->MatchScan(
        pScan, m_pMapperSensorManager->GetRunningScans(pScan->GetSensorName()), bestPose, cov);
      pScan->SetSensorPose(bestPose);
      if (covariance)
      {
        *covariance = cov;
...
kt_bool Mapper::ProcessAgainstNodesNearBy(LocalizedRangeScan* pScan,
                                          kt_bool addScanToLocalizationBuffer, Matrix3* covariance)
{
  if (pScan != NULL)
  {
    karto::LaserRangeFinder* pLaserRangeFinder = pScan->GetLaserRangeFinder();
    ...
    Vertex<LocalizedRangeScan>* closetVertex =
      m_pGraph->FindNearByScan(pScan->GetSensorName(), pScan->GetOdometricPose());
...
void Mapper::AddScanToLocalizationBuffer(LocalizedRangeScan* pScan,
...
bool Mapper::cutsLocalizationAndMappingGraphs(const LocalizationScanVertex& localization_vertex)
{
  ...
  // really big. but for now I use a simpler, but naive, approach where I check the localization
  // vertices to see if both of these conditions are true:
...
kt_bool Mapper::RemoveNodeFromGraph(Vertex<LocalizedRangeScan>* vertex_to_remove)
{
  ...
  std::vector<Vertex<LocalizedRangeScan>*> adjVerts = vertex_to_remove->GetAdjacentVertices();
  for (int i = 0; i != adjVerts.size(); i++)
  {
    ...
    for (int j = 0; j != adjEdges.size(); j++)
    {
          ...
          adjEdges[j]->GetSource() == vertex_to_remove)
      {
        ...
        m_pScanOptimizer->RemoveConstraint(adjEdges[j]->GetSource()->GetObject()->GetUniqueId(),
  ...
  std::map<Name, std::map<int, Vertex<LocalizedRangeScan>*>> vertexMap = m_pGraph->GetVertices();
  std::map<int, Vertex<LocalizedRangeScan>*> graphVertices =
    vertexMap[vertex_to_remove->GetObject()->GetSensorName()];
  std::map<int, Vertex<LocalizedRangeScan>*>::iterator vertexGraphIt =
    graphVertices.find(vertex_to_remove->GetObject()->GetStateId());
...
kt_bool Mapper::HasMovedEnough(LocalizedRangeScan* pScan, LocalizedRangeScan* pLastScan) const
{
  ...
  kt_double deltaHeading =
    math::NormalizeAngle(scannerPose.GetHeading() - lastScannerPose.GetHeading());
...
{
  std::vector<MapperListener*>::iterator iter =
    std::find(m_Listeners.begin(), m_Listeners.end(), pListener);
